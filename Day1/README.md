...
Let's create the load balancer container, in the below command the port 80 on the left has to be different for different participant. You can change it to maybe 81,82 or any port available on the local server ( 100 to 65535 ).  The port on the right side is configured to be 8080 within the bitnami/nginx:latest image,hence we can't change without creating a custom docker image.
```
